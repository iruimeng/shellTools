...
const (
	...
	STARTURL    string = "http://www.mzitu.com/xinggan" //妹子图模块列表页url
...
func getList(url string) (l []string) {
	reg, _ := regexp.Compile(`<span><a href="(http://www.mzitu.com/\d*)" target="_blank">.*?</a></span>`)
...
//获取最大分页
func getPage(url string, span bool) (page int) {
	fmt.Println(url)
	_, html, _ := getHtml(url)
	...
	if span {
		...
		reg, _ = regexp.Compile(`<span class="meta-nav screen-reader-text"></span>(\d*)<span class="meta-nav screen-reader-text"></span>`)
	}
...
func download(url string) {
	...
	reg1, _ := regexp.Compile(`http:\/\/pic\.mmfile\.net/(.*)`)
